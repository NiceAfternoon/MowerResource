...
def skip_in_workflow(func):
    def wrapper(*args, **kwargs):
        if IS_WORKFLOW:
            print(f"检测到当前为 Workflow 环境，已跳过: {func.__name__}")
            return None
...
class Arknights数据处理器:
    ...
    def 添加干员(self):
        ...
        with open(
            f"{RESOURCE_ROOT}/arknights_mower/data/agent_profession.json", "w", encoding="utf-8"
        ) as f:
    ...
    def 读取活动关卡(self):
        ...
        for item in self.常驻关卡:
            ...
            unkey += 1
        with open(
            f"{RESOURCE_ROOT}/ui/src/pages/stage_data/event_data.json", "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            f"{RESOURCE_ROOT}/arknights_mower/data/stage_data_full.json", "w", encoding="utf-8"
        ) as f:
    ...
    def load_recruit_data(self):
        ...
        with open(
            f"{RESOURCE_ROOT}/arknights_mower/data/recruit_result.json", "w", encoding="utf-8"
        ) as f:
    ...
    def load_recruit_tag(self):
        ...
        font = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 30
        )
    ...
    def 批量训练并保存扫仓库模型(self):
        ...
        # self.训练仓库的knn模型("MATERIAL", f"{RESOURCE_ROOT}/arknights_mower/models/MATERIAL.pkl")

    def 训练在房间内的干员名的模型(self):
        font = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 37
        )
    ...
    def 训练选中的干员名的模型(self):
        font31 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 31
        )
        font30 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 30
        )
        font25 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 25
        )
        font23 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 23
        )

        font27 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 27
        )
    ...
    def 训练训练室干员名的模型(self):
        ...
        font28 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 28
        )
        font25 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 25
        )
        ...
        font24 = ImageFont.truetype(
            f"{RESOURCE_ROOT}/arknights_mower/fonts/SourceHanSansCN-Medium.otf", 24
        )
    ...
    def 获得干员基建描述(self):
        ...
        # print(干员技能列表)
        with open(
            f"{RESOURCE_ROOT}/ui/src/pages/basement_skill/skill.json", "w", encoding="utf-8"
        ) as f:
    ...
    def buff转换(self):
        ...
        with open(
            f"{RESOURCE_ROOT}/ui/src/pages/basement_skill/buffer.json", "w", encoding="utf-8"
        ) as f:
    ...
    def generate_version_info(self):
            ...
            # 初始化version
            version_path = os.path.join(RESOURCE_ROOT, "version.json")
            ...
            for activity in basic_info.values():
                ...
                if a_end >= current_time:
                    ...
                    a_filterlist = ["CHECKIN", "ONLY", "COLLECTION"]
